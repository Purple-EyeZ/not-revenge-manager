package app.notrevenge.manager.installer.session
...
import app.notrevenge.manager.ui.activity.MainActivity
...
class InstallService : Service() {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        val isInstall = intent.action == "notrevenge.actions.ACTION_INSTALL"
        when (val statusCode = intent.getIntExtra(PackageInstaller.EXTRA_STATUS, -999)) {
            ...
            PackageInstaller.STATUS_FAILURE_ABORTED -> if (isInstall) showToast(R.string.installer_aborted)
                ...
                if (isInstall) {
                    ...
                    startActivity(
                        Intent("notrevenge.actions.ACTION_INSTALL_FINISHED").apply {
